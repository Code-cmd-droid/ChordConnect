...
public class DashboardFrame extends JFrame {
    ...
    private int currentRoomId; // ADD THIS MISSING VARIABLE

    public DashboardFrame(int userId, String username) {
        this.userId = userId;
        this.username = username;
        this.currentRoomId = generateRoomId(userId, (int) (System.currentTimeMillis() % 10000)); // INITIALIZE IT
        ...
        JButton videoCallBtn = new JButton("🎥 Start Video Call");
        styleVideoCallButton(videoCallBtn);
        videoCallBtn.addActionListener(e -> startVideoCall());
        videoCallBtn.setAlignmentX(Component.CENTER_ALIGNMENT);
        ...
        centerContent.add(videoCallBtn);
        ...
        // ---------- FRIENDS LIST ----------
        JPanel friendsPanel = createListPanel("Friends List", fetchFriends(), "Video Call");
        ...
        // ---------- RECENT JAMMERS ----------
        JPanel jammersPanel = createListPanel("Recent Jammers", fetchRecentJammers(), "Join Call");
    ...
    // --- Style video call button ---
    private void styleVideoCallButton(JButton btn) {
        btn.setFont(new Font("Segoe UI", Font.BOLD, 24));
        btn.setBackground(new Color(66, 133, 244)); // Google Meet blue
        ...
        btn.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseEntered(java.awt.event.MouseEvent evt) {
                btn.setBackground(new Color(53, 119, 229));
            }
            public void mouseExited(java.awt.event.MouseEvent evt) {
    ...
    private void startVideoCall() {
        try {
            ...
            int roomId = generateRoomId(userId, (int) (System.currentTimeMillis() % 10000));

            // Open browser for video call - FIXED: pass roomId parameter
            openVideoCallInBrowser(roomId);

            JOptionPane.showMessageDialog(this,
                    "🎥 Video call opening in browser!\n" +
                            "📍 Room ID: " + roomId + "\n" +
            ...
            JOptionPane.showMessageDialog(this,
                    "Failed to start video call: " + ex.getMessage() +
    ...
    // --- Join video call with specific user ---
    private void joinVideoCallWithUser(String targetUsername) {
        try {
            // Look up the user from backend
            JSONObject response = ApiClient.getUserByUsername(targetUsername);
            if (response.getBoolean("success")) {
                JSONObject userData = response.getJSONObject("data");
                int targetUserId = userData.getInt("id");

                // Generate room ID for both users
                int roomId = generateRoomId(userId, targetUserId);

                // Open browser for video call - FIXED: pass roomId parameter
                openVideoCallInBrowser(roomId);

                JOptionPane.showMessageDialog(this,
                        "🎥 Starting video call with " + targetUsername + "!\n" +
                                "📍 Room ID: " + roomId + "\n" +
                                "🔗 Send this room ID to " + targetUsername + " to join",
                        "Video Call",
                        JOptionPane.INFORMATION_MESSAGE);
            } else {
                JOptionPane.showMessageDialog(this,
                        "User '" + targetUsername + "' not found!\n" +
                                "Starting a general video call instead.",
                        "User Not Found",
                        JOptionPane.WARNING_MESSAGE);

                // Fallback to general video call
                startVideoCall();
            }
        } catch (Exception e) {
            JOptionPane.showMessageDialog(this,
    ...
    // --- Helper method to open browser - FIXED: ADD roomId PARAMETER ---
    ...
    private JPanel createListPanel(String title, List<String> names, String actionLabel) {
        ...
        if (names.isEmpty()) {
            ...
            for (String name : names) {
                ...
                actionBtn.setBackground(new Color(66, 133, 244)); // Google Meet blue
                ...
                if ("Video Call".equals(actionLabel)) {
                    actionBtn.addActionListener(e -> joinVideoCallWithUser(name));
                } else if ("Join Call".equals(actionLabel)) {
                    actionBtn.addActionListener(e -> joinVideoCallWithUser(name));
                } else if ("Message".equals(actionLabel)) {
    ...
    private void showProfileDialog() {
        ...
        // Add video call quick access button
        JButton quickVideoCallBtn = new JButton("🎥 Quick Video Call");
        styleButton(quickVideoCallBtn, new Color(66, 133, 244));
        quickVideoCallBtn.addActionListener(e -> {
            dialog.dispose();
            startVideoCall();
        });
    ...
    private List<String> fetchRecentJammers() {
        ...
        try {
            JSONObject response = ApiClient.getRecentJammers(userId);
            if (response.getBoolean("success")) {
