...
public class ChatWindow extends JFrame {
    ...
    private JPanel createControlsPanel() {
        ...
        // Video call controls
        startVideoCallBtn = new JButton("🎥 Start Video Call");
        endVideoCallBtn = new JButton("❌ End Video Call");
...
// In the startVideoCall method, update this line:
    ...
    public void startVideoCall() {
        if (!videoCallActive) {
            try {
                ...
                startVideoCallBtn.setEnabled(false);
                endVideoCallBtn.setEnabled(true);

                // For group calls - allow anyone to join
                videoClient = new VideoCallClient(userId, username, currentRoomId, videoPanel);
                videoClient.startCall();

                // Connect to video signaling
                setupVideoSignaling();

                String callMessage = "[" + new java.util.Date() + "] 🎥 Group jam session started!\n" +
                        "[" + new java.util.Date() + "] Anyone in this room can join the video call\n";
                appendToChat(callMessage);

            } catch (Exception e) {
                JOptionPane.showMessageDialog(this, "Failed to start video call: " + e.getMessage());
                endVideoCall();
            }
        }
    }
    ...
    private void endVideoCall() {
        if (videoCallActive) {
            ...
            startVideoCallBtn.setEnabled(true);
            endVideoCallBtn.setEnabled(false);
            ...
            if (videoSignalingClient != null) {
                videoSignalingClient.disconnect();
                videoSignalingClient = null;
            }

            appendToChat("[" + new java.util.Date() + "] 🎥 Video call ended\n");
        }
    }
